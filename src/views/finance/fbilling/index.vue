<template>
  <div class="app-container">
      ...
      <el-row>
        ...
        <el-col :xs="8" :sm="8" :md="6">
          <el-form-item label="发生日期" prop="amountTime">
            <el-date-picker clearable v-model="queryParams.amountTime" type="datetime"
              value-format="YYYY-MM-DD HH:mm:ss" placeholder="请选择费用发生日期">
            </el-date-picker>
          </el-form-item>
        </el-col>
        <el-col :xs="8" :sm="8" :md="6">
        ...
        <el-col :xs="8" :sm="8" :md="6">
          <el-form-item label="付费方式" prop="paymentMethod">
            <el-select v-model="queryParams.paymentMethod" placeholder="请选择付费方式" clearable>
        ...
        <el-col :xs="8" :sm="8" :md="6">
          <el-form-item label="来源系统" prop="sourceSys">
            <el-select v-model="queryParams.sourceSys" placeholder="请选择来源系统">
        ...
        <el-col :xs="8" :sm="8" :md="6">
          <el-form-item label="费用项目" prop="expenseItemId">
            <el-select v-model="queryParams.expenseItemId" placeholder="请选择费用项目" clearable>
              <el-option v-for="item in expenseOptions" :key="item.value" :label="item.label" :value="item.value" />
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :xs="8" :sm="8" :md="6">
          <el-form-item label="费用类型" prop="type">
            <el-select v-model="queryParams.type" placeholder="请选择费用类型" clearable>
              <el-option v-for="dict in bms_expense_type" :key="dict.value" :label="dict.label" :value="dict.value" />
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :xs="8" :sm="8" :md="6">
          <el-form-item label="币种" prop="currencyId">
            <el-select v-model="queryParams.currencyId" placeholder="请选择币种" clearable>
        ...
        <el-col :xs="8" :sm="8" :md="6">
          <el-form-item label="创建账单" prop="isCreateBill">
            <el-select v-model="queryParams.isCreateBill" placeholder="请选择是否创建账单" clearable>
        ...
        <el-col :xs="8" :sm="8" :md="6">
          <el-form-item label="开发票" prop="isInvoiced">
            <el-select v-model="queryParams.isInvoiced" placeholder="请选择是否开发票" clearable>
              <el-option v-for="dict in business_yes_no" :key="dict.value" :label="dict.label" :value="dict.value" />
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :xs="8" :sm="8" :md="6">
          <el-form-item label="是否挂账" prop="isAccounted">
            <el-select v-model="queryParams.isAccounted" placeholder="请选择是否挂账" clearable>
              <el-option v-for="dict in business_yes_no" :key="dict.value" :label="dict.label" :value="dict.value" />
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :xs="8" :sm="8" :md="6">
          <el-form-item label="是否审核" prop="isApproved">
            <el-select v-model="queryParams.isApproved" placeholder="请选择是否审核" clearable>
              <el-option v-for="dict in business_yes_no" :key="dict.value" :label="dict.label" :value="dict.value" />
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :xs="8" :sm="8" :md="6">
          <el-form-item label="是否锁定" prop="isLocked">
            <el-select v-model="queryParams.isLocked" placeholder="请选择是否锁定" clearable>
              <el-option v-for="dict in business_yes_no" :key="dict.value" :label="dict.label" :value="dict.value" />
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :xs="8" :sm="8" :md="6">
          <el-form-item label="是否提交" prop="isSubmit">
            <el-select v-model="queryParams.isSubmit" placeholder="请选择是否提交" clearable>
              <el-option v-for="dict in business_yes_no" :key="dict.value" :label="dict.label" :value="dict.value" />
            </el-select>
          </el-form-item>
        </el-col>
    ...
    <el-table v-loading="loading" :data="expensesList" @selection-change="handleSelectionChange" height="680px" border>
      ...
      <el-table-column label="费用状态" align="center" prop="expenseStatus" width="100">
        <template #default="scope">
      ...
      <el-table-column label="创建账单" align="center" prop="isCreateBill">
        <template #default="scope">
          <dict-tag :options="business_yes_no" :value="scope.row.isCreateBill" />
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" v-model="open" width="660px" append-to-body>
      <el-form ref="expensesRef" :model="form" :rules="rules" label-width="80px">
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="费用项目" prop="expenseItemId">
              <el-select v-model="form.expenseItemId" placeholder="请选择费用项目">
                <el-option v-for="item in expenseOptions" :key="item.value" :label="item.label" :value="item.value" />
              </el-select>
            </el-form-item>
          </el-col>
...
const { base_expense_payment_type, bms_expense_type, bms_expense_status, bms_payment_method, bms_source_sys, business_yes_no } = proxy.useDict('base_expense_payment_type', 'bms_expense_type', 'bms_expense_status', 'bms_payment_method', 'bms_source_sys', 'business_yes_no');
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除费用清单编号为"' + _ids + '"的数据项？').then(function () {
...
function handleExport() {
  proxy.download('bms/expenses/export', {
    ...queryParams.value
